...
class AmpCorsMiddleware implements HttpKernelInterface
{
     ...
     * @return array
     */
    private function createValidOrigins($publisherOrigin)
    {
        return [
            $publisherOrigin,
            sprintf('%s.cdn.ampproject.org', str_replace('.', '-', str_replace('-', '--', $publisherOrigin))),
            sprintf('%s.amp.cloudflare.com', $publisherOrigin),
    ...
    private function retrieveOrigin(Request $request, $ampSourceOrigin)
    {
        if ($this->requestContainsSameOrigin($request)) {
            ...
            return $request->headers->get(self::ORIGIN_HEADER);
        }
    ...
    {
        $validOrigins = $this->createValidOrigins($this->publisherOrigin);
